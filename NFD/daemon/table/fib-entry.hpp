...
public:
	uint32_t
	getForwarderId() const;

	void
	setForwarderId(const uint32_t id);

	name_tree::Entry*
	getNameTreeEntry() const;
/******* DMIF *******/
...
private:
 ...
 uint32_t m_DMIF_InterestOrData=0;
  
 clock_t  m_DMIF_StampTime;
...
/******* BF ********/
 uint32_t m_BF_forwarderId;
 
 uint32_t m_BF_InterestOrData=0;
  
  clock_t  m_BF_StampTime;
