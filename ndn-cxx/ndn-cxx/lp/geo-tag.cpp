...
{
  size_t length = 0;
  length += prependNonNegativeIntegerBlock(encoder, tlv::GeoTagPos, std::get<2>(m_pos));
  length += prependNonNegativeIntegerBlock(encoder, tlv::GeoTagPos, std::get<1>(m_pos));
  length += prependNonNegativeIntegerBlock(encoder, tlv::GeoTagPos, std::get<0>(m_pos));
...
GeoTag::wireDecode(const Block& wire)
{
  ...
  m_pos = {encoding::readNonNegativeInteger(m_wire.elements()[0]),
           encoding::readNonNegativeInteger(m_wire.elements()[1]),
           encoding::readNonNegativeInteger(m_wire.elements()[2])};
}
